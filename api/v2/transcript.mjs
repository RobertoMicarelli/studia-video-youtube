 ...
 */
/** "en-US" -> "en" */
const codiceLingua = (l) => (l || '').split('-')[0].toLowerCase() || null;

async function chiamaProvider(key, videoId, lang) {
  ...
  const res = await fetchT(
    `https://api.supadata.ai/v1/transcript?${qs}`,
    { headers: { 'x-api-key': key } },
    20000
  );
  if (!res.ok) {
    const detail = await res.text().catch(() => '');
    const err = new Error(`provider HTTP ${res.status} ${detail.slice(0, 120)}`);
...
async function viaProvider(videoId, linguaOriginale) {
  const key = process.env.SUPADATA_API_KEY;
  if (!key) return null;

  // Senza `lang` il provider restituisce una traccia qualunque fra quelle
  // disponibili: su video inglesi tornava l'ARABO, cioe' una traduzione
  // automatica. Peggiora la dispensa, perche' si traduce una traduzione.
  // Chiediamo la lingua ORIGINALE del video, la piu' fedele: al modello poi
  // il prompt impone comunque di scrivere in italiano.
  let data;
  try {
    data = await chiamaProvider(key, videoId, linguaOriginale);
  } catch (err) {
    // La lingua richiesta puo' non essere disponibile: un solo ritento libero,
    // per non bruciare richieste del piano (100/mese).
    if (!linguaOriginale) throw err;
    data = await chiamaProvider(key, videoId, null);
  }
  ...
  return {
    ...
    availableLangs: data.availableLangs || null,
    richiesta: linguaOriginale || '(nessuna preferenza)'
  };
}

async function extractTranscript(videoId, linguaOriginale) {
  const attemptsPre = [];

  // Il provider ha la precedenza: e' l'unico che oggi restituisce testo davvero.
  if (process.env.SUPADATA_API_KEY) {
    try {
      const p = await viaProvider(videoId, linguaOriginale);
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // --- Trascrizione ---
    // La lingua originale dichiarata dal video guida la scelta della traccia:
    // senza, il provider puo' restituire una traduzione automatica.
    const linguaOriginale =
      codiceLingua(v.snippet.defaultAudioLanguage) || codiceLingua(v.snippet.defaultLanguage);

    const { transcript, source, language, autoGenerated, availableLangs, langRichiesta, attempts } =
      await extractTranscript(videoId, linguaOriginale);
    ...
    return res.status(200).json({
      ...
      // Diagnostica sulla scelta della traccia
      linguaOriginale: linguaOriginale || null,
      langRichiesta: langRichiesta || null,
      availableLangs: availableLangs || null,
      attempts
    });
  } catch (error) {
